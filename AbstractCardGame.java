...
//	}
	...
	/**
	 * Clone Card Game by creating a new version
	 */
	public CardGame clone() {
		AbstractCardGame cloned = null;
		//Find out what version of whist is being played
		if(this instanceof ClassicWhist) {
			cloned = new ClassicWhist();
		} else if(this instanceof KnockOutWhist) {
			cloned = new KnockOutWhist();
		} else if(this instanceof SingleHandWhist) {
			cloned = new SingleHandWhist();
		}
		//Clone each player and set them to the cloned players
		for(Player.Direction pd : players.keySet()) {
			cloned.players.put(pd,players.get(pd).clone());
		}
		//Clone number of tricks won for each person
		for(Player.Direction pd : tricks.keySet()) {
			cloned.tricks.put(pd,tricks.get(pd).intValue());
		}
		//Clone scores for each person
		for(Player.Direction pd : scores.keySet()) {
			cloned.scores.put(pd,scores.get(pd).intValue());
		}
		cloned.trumps = trumps;
		//If part way through a trick then clone that
		if(currentTrick == null) {
			cloned.currentTrick = null;
		} else {
			cloned.currentTrick = currentTrick.clone();
		}
		return cloned;
	}
