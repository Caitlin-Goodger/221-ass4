...
public class Trick {
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		//Check that it is the right person playing
		if(!p.direction.equals(getNextToPlay())) {
			throw new IllegalMove("Not your turn");
		}
		//Check that they are following suit if they have to 
		if(cards[0] != null&& p.hand.matches(cards[0].suit()).size() >0 && !c.suit().equals(cards[0].suit())) {
			throw new IllegalMove("Illegal Cards");
		}
	...
	/**
	 * Clone the current trick and clone the cards being used
	 */
	public Trick clone() {
		return new Trick(lead,trumps,cards.clone());
	}
}
